...
def mongodb_conn(mocker):
    ...
    mock_collection.insert_one = mocker.MagicMock(return_value=mocker.MagicMock(inserted_id="507f1f77bcf86cd799439011"))
    mock_collection.find = mocker.MagicMock(return_value=[{"test": "value"}])
    mock_collection.delete_many = mocker.MagicMock(return_value=mocker.MagicMock(deleted_count=1))
    mock_collection.update_one = mocker.MagicMock(return_value=mocker.MagicMock(modified_count=1))
